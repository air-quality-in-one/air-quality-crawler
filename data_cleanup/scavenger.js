...
function cleanup () {
	...
	queue.onJob(function (job, done) {
		...
		console.log("Finish clean up overdue data!");
	}).start();
}
...
function removeOverdueData(day, done) {
	AirQuality.prepareDataXDaysBefore(day, function (error, result) {
      ...
      doRemoval(result, function (err) {
      			if (err) {
      				console.log("Fail to clean up quality data " + day + " days before!");
							return done("remove error");
      			} else {
      				console.log("Success to clean up quality data " + day + " days before!");
							return done(null);
      			}
      		});
		}
